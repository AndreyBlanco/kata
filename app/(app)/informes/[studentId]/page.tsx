...
function InformePreviewContent() {
  const router = useRouter()
  const params = useParams()
  const searchParams = useSearchParams()
  const studentId = params.studentId as string
  const months = searchParams.get('months') || ''
  const schoolPeriod = searchParams.get('schoolPeriod') || ''
  ...
  useEffect(() => {
    async function loadReport() {
      try {
        ...
        if (schoolPeriod) q.set('schoolPeriod', schoolPeriod)
        const res = await fetch(`/api/reports/${studentId}?${q.toString()}`)
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (schoolPeriod) q.set('schoolPeriod', schoolPeriod)
      const res = await fetch(`/api/reports/export/${studentId}?${q.toString()}`)
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <button
            onClick={() => router.back()}
            className="text-kata-primary hover:underline"
          >
            ← Volver
          </button>
        </div>
...
export default function InformePreviewPage() {
  return (
    <Suspense fallback={<LoadingState message="Generando informe..." />}>
      <InformePreviewContent />
    </Suspense>
  )
}