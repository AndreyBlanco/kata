...
export default function ValoracionesPage() {
  ...
  const filterBtn = (f: Filter, label: string, count: number) => (
    ...
    >
      {label} <span className={`ml-1 text-xs ${filter === f ? 'text-white/80' : 'text-gray-400'}`}>({count})</span>
    </button>
  )
  ...
  return (
    <>
      <PageHeader
        title="Valoraciones integrales"
        subtitle={`${counts.active} en proceso · ${counts.completed} completadas`}
      >
        <Button
          variant="primary"
